...
#endif
IDBin::IDBin(QObject *parent) : QObject(parent), setTimerCount_(0), coldStartTimerCount_(0), isColdStart_(false), // for unique
...
Resp IDBin::parse(FrameParser &proto) {
    ...
    if(proto.type() == CONTENT || proto.type() == SETTING || proto.type() == GETTING) {
        ...
        if ((proto.resp() && proto.type() == CONTENT) || proto.type() == GETTING) {
        ...
        else if ((!proto.resp() && proto.type() == CONTENT) ||  proto.type() == SETTING) {
            m_lastResp = respNone;
...
Resp IDBinAttitude::parsePayload(FrameParser &proto) {
    qDebug() << "IDBinAttitude::parsePayload...........";
    if(proto.ver() == v0) {
...
Resp IDBinDataset::parsePayload(FrameParser &proto) {
    qDebug() << "IDBinDataset::parsePayload............";
    if (proto.ver() == v0) {
